...
const LINT_MESSAGE_REGEX =
  /L (\d+) \(C (\d+)-?(\d*)\): ([\dA-Za-z]+): ML(\d): (.*)/;
const FIX_FLAG_REGEX = /\(CAN FIX\)/;
const FIX_MESSAGE_REGEX = /----FIX MESSAGE<\w+>\s+<([^>]*)>/;
const FIX_CHANGE_REGEX =
  /----CHANGE MESSAGE L (\d+) \(C (\d+)\);\s+L (\d+) \(C (\d+)\):\s+<([^>]*)>/;
...
class LintingSupportProvider {
  ...
  private readonly SUPPRESS_DIAGNOSTIC_REQUEST_CHANNEL =
    "/matlabls/linting/suppressdiagnostic/request";
  private readonly SUPPRESS_DIAGNOSTIC_RESPONSE_CHANNEL =
    "/matlabls/linting/suppressdiagnostic/response";
  ...
  async lintDocument(
    ...
    const isMatlabAvailable =
      matlabConnection != null && MatlabLifecycleManager.isMatlabReady();
    ...
    if (isMatlabAvailable) {
      ...
      lintData = await this.getLintResultsFromMatlab(
        code,
        fileName,
        matlabConnection
      );
  ...
  handleCodeActionRequest(params: CodeActionParams): CodeAction[] {
    ...
    codeActions = codeActions.filter((action) => {
      ...
      return params.context.diagnostics.some((diag) =>
        this.isSameDiagnostic(diagnostic, diag)
      );
    });
    ...
    commands.forEach((command) => {
      ...
      codeActions.push(
        CodeAction.create(command.title, command, CodeActionKind.QuickFix)
      );
    });
  ...
  suppressDiagnostic(
    ...
    range: Range,
    id: string,
    shouldSuppressThroughoutFile: boolean
  ): void {
    const matlabConnection = MatlabLifecycleManager.getMatlabConnection();
    ...
    const responseSub = matlabConnection.subscribe(
      this.SUPPRESS_DIAGNOSTIC_RESPONSE_CHANNEL,
      (message) => {
        matlabConnection.unsubscribe(responseSub);

        const suppressionEdits: TextEdit[] = (
          message as DiagnosticSuppressionResults
        ).suppressionEdits;
   ...
   */
  private async getLintResultsFromMatlab(
    code: string,
    fileName: string,
   ...
   */
  private async getLintResultsFromExecutable(
    fileName: string
  ): Promise<string[]> {
    ...
    const mlintArgs = [fileName, "-id", "-severity", "-fix"];

    return await new Promise<string[]>((resolve) => {
      try {
        exec(
          `"${'D:/Program Files/MATLAB/R2016b/bin/win64/mlint'}"` + " " + mlintArgs.join(" "),
          { encoding: "binary" },
          (error: ExecFileException | null, stdout: string, stderr: string) => {
            if (error != null) {
              Logger.error(
                `Error from mlint executable: ${error.message}\n${
                  error.stack ?? ""
                }`
              );
              resolve([]);
            }
            console.log(iconv.decode(Buffer.from(stderr, "binary"), "cp936"));
            resolve(iconv.decode(Buffer.from(stderr, "binary"), "cp936").split("\n")); // For some reason, mlint appears to output on stderr instead of stdout
          }
        );
      } catch (e) {
  ...
  private async getMlintExecutable(): Promise<string | null> {
    ...
    // const matlabInstallPath = (await ConfigurationManager.getConfiguration())
    //   .installPath;
    const matlabInstallPath = ""
    let binPath = "";
    ...
    const mlintExecutablePath = path.normalize(
      path.join(binPath, platformDir, "mlint")
    );
  ...
  private processLintResults(
    uri: string,
    lintData: string[]
  ): { diagnostics: Diagnostic[]; codeActions: CodeAction[] } {
    ...
    while (dataIndex < lintData.length) {
      ...
      const endColumn =
        parsedLine[3] !== "" ? parseInt(parsedLine[3]) : startColumn + 1; // +1 for open interval
      ...
      const diagnostic = Diagnostic.create(
        Range.create(line, startColumn, line, endColumn),
        lintMessage,
        severity,
        id,
        "MATLAB"
      ...
      while (dataIndex < lintData.length) {
        ...
        if (startLine === endLine && startColumn === endColumn) {
          ...
          edit = TextEdit.insert(
            Position.create(startLine, startColumn + 1),
            replaceText
          );
